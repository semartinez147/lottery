...
public class Lottery {
  ...
  public Lottery(int poolSize){
    this(poolSize, new SecureRandom()); // looks for another constructor that takes both parameters

  }

  public int[] draw(int sampleSize) {
    if (sampleSize > pool.length) {
      throw new IllegalArgumentException("Sample size must not exceed pool");
    }
